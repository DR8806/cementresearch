...
start_year <- 1971 #we are working with 1971 - 2021 data now
...
var_model_lim <- VAR(differenced_df_lim, p = 1, type = "both") # need to justify why we can use the "both" parameter
...
fanchart(forecast_lim, 
         main = c("Cement","Unemployment","Housing", "Limestone","GDP")
         )
...
stepwise_vals <- bind_cols(data.frame(seq(start_year,2050), 
...
mdl.norm <- normality.test(var_model_lim,multivariate.only = TRUE)
...
#Impulse Response 
irf_results <- irf(var_model_lim, n.ahead = 29, response = c("unemploy", "gdp","housing","limestone"))
